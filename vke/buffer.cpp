...
Buffer::Buffer(Core* core, VkBufferUsageFlagBits usage, usize buffer_size, bool host_visible)
    : Resource(core) {
...
std::unique_ptr<Buffer> Core::create_buffer(VkBufferUsageFlagBits usage, usize buffer_size, bool host_visible /* whether it is accessible by cpu*/) {
    return std::make_unique<Buffer>(this, usage, buffer_size, host_visible);
}

// creates host visible data initalized with given bytes
std::unique_ptr<Buffer> Core::create_buffer_from_data(VkBufferUsageFlagBits usage, std::span<u8> bytes) {
